...
class BenefactorRegistration(generics.CreateAPIView):
    ...
    def post(self, request):
        srz_data = BenefactorSerializer(data=request.POST)
...
class CharityRegistration(generics.CreateAPIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        srz_data = CharitySerializer(data=request.POST)
        if srz_data.is_valid():
            srz_data.save(user=request.user)
            return Response(srz_data.data, status=status.HTTP_201_CREATED)
        return Response(srz_data.errors, status=status.HTTP_400_BAD_REQUEST)
...
class TaskRequest(APIView):
    ...
    def get(self, request, task_id):
        ...
        # assigned this task to user
        Task.assign_to_benefactor(task, request.user.benefactor)
...
class TaskResponse(APIView):
    ...
    def post(self, request, task_id):
        r_or_a = request.data.get('response')
        if (r_or_a != 'R') and (r_or_a != 'A'):
...
class DoneTask(APIView):
    ...
    def post(self, request, task_id):
        task = get_object_or_404(Task, pk=task_id)
        if (task) and (task.state != Task.TaskStatus.ASSIGNED):
